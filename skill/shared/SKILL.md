...
| `/task plan-approve #N`          | Record explicit human Plan approval on a Plan issue. Writes `<!-- aitm-plan-approved: <ts> -->` into the issue body — required before `/task promote` can advance the issue from Plan to Develop. Idempotent. Only valid while the issue is in Plan. |
| `/task approve #N`               | Record explicit human review approval on a Review issue. Writes `<!-- aitm-review-approved: <ts> -->` into the issue body that `/task close` requires when `gateReviewToDone=true`. Idempotent. Only valid while the issue is in Review.             |
...
`/task plan-approve #N` (records Plan approval; gates the Plan → Develop promotion), `/task approve #N` (records Review approval; gates the Review → Done promotion), and `/task reject #N --reason "..."` (Review → Develop with rejection reason) remain first-class — they are not state-walking verbs.
