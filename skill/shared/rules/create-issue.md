...
Required content fragments (default `./.scratch/plan/`): `scope.md`, `acs.md` (must contain `- [ ]` checkboxes), and `story-origin.md` with create-time provenance. `user-story.md` is optional before Plan approval. When supplied, it contains either the exact canonical template or three non-empty, heading-free Connextra lines beginning `As a`, `I want to`, and `So that`; substantive prose must pass draft validation. When omitted, the renderer preserves the normal empty `## User Story` section. Plan Metadata in `plan-meta.md` is optional until planning produces substantive output. For sub-issues, also pass `--parent <EPIC_N>`. Load `rules/user-story-quality.md` for the shared lifecycle and review contract.

Every non-stub shaped call passes `--scope-file ./.scratch/plan/scope.md`, `--ac-file ./.scratch/plan/acs.md`, and `--story-origin-file ./.scratch/plan/story-origin.md`. Add `--user-story-file ./.scratch/plan/user-story.md` only when draft story input exists; append `--plan-metadata-file ./.scratch/plan/plan-meta.md` only when planning output already exists.

## Defect intent routes to the defect shape

When the user asks to create, generate, or file a local defect or bug story, use `--shape defect`. It has the same required Scope, Acceptance Criteria, and Story Origin fragments, and the same optional early User Story input, as `solo`; optional `--reproduction-file`, `--root-cause-file`, `--fix-direction-file`, and `--out-of-scope-file` fragments capture diagnostic detail. The wrapper adds the `bug` label and canonical `🐞 [BUG]` title prefix idempotently.
