> Historical human reference preserved during #1773 protocol slimming. Current CLI results and `rules/review.md` govern actions; older approval wording below describes the manual path and is not automatically loaded instruction authority.
...
**Full-Auto.** With `TT_FULL_AUTO=1`, current AITM policy may record `aitm-full-auto-approved` and its visible footnote. Its provenance differs from explicit `--human` approval. The current CLI decision and Review rule govern Close readiness.
