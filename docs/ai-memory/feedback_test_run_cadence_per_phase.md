...
**Why:** each run has a distinct job. Develop = fast per-AC feedback. Test = "is it actually checked in and CI-runnable" in isolation. Review/cloud = the real merge gate. Running `test:all` multiple times *within* one phase (my #739 mistake — 2–3× inside Test) is pure waste, not extra safety.
