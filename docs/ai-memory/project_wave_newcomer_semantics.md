...
---
In the 8-state kanban Sequence-driven wave model (issue #41 epic, extended by #433), when a deep-dive discovers a new sub-issue and slots it into the *current* wave (same `Sequence` as already-flowing members), the gate behavior is asymmetric:
...
**How to apply:** When implementing sub-issue 3 (Grooming → Analysis gate) and sub-issue 8 (E2E test) of issue #41, ensure the wave-admission predicate operates only on the *epic-level next-wave* admission check, not on the per-issue forward-state transitions. The E2E test should explicitly cover: a wave-1 member already in Review, a wave-1 newcomer just entering Refine, and verify that (a) the in-flight member can still move Review → Done, and (b) wave-2 admission stays blocked until the newcomer also reaches Done.
