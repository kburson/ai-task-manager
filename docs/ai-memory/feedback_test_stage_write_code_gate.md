...
**Two guards now enforce this, and both must allow for an edit to land (#805).** `activity-guard.mjs` reads the *mirrored* `active-task.json#kanbanState` (a lagging copy of the body's `aitm-last-known-state`), so on mirror-drift it could fall open. `source-edit-gate.mjs`'s `decideSourceEdit` now carries a **fail-closed post-develop lock**: for the post-develop states `{test, review, done}` it classifies the edit with the shared `classifyEdit`/`isAllowed` matrix and refuses any class the `STATE_MATRIX` no longer permits (WRITE_CODE in all three; WRITE_DOCS still passes in `review`). Because `source-edit-gate` resolves state from the **live board** (`gh issue view`, 30s cache), the refusal holds even when the mirror lags. Before #805 this contract was documentation + operator discipline only; it is now tool-enforced — a `test`-state source/`*.test.mjs` edit is refused with code `source-edit-post-develop-lock` and a message naming the `demote → fix → verify-develop → re-promote` loop. The mirror-staleness in `activity-guard` (H2) remains a separately-tracked drift class ([[project_marker_after_verified_move]]).
