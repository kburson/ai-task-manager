...
When an issue's deliverable is NOT source code — a repo-config change (e.g. creating a GitHub label), a decision, or the *action from* a spike finding — it produces zero commits. The develop→test code lane then breaks: `/task commit-trace` has no commit to record, so it grabs **HEAD**, which is some *other* issue's commit, and stamps a false `### 🔗 Commits` trail.
