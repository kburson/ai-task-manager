...
| `kanbanOptionBacklog` / `Refine` / `Plan` / `Develop` / `Test` / `Review` / `Done` (canonical config keys, one per state slug; the project-tether maps state slugs onto them — see `scripts/gh/lib/project-tether.mjs` `STATUS_CONFIG_KEYS`) | Kanban option IDs                            |
