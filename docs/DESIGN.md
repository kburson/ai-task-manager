...
| `kanbanOptionBacklog` / `Groom` / `Analyze` / `Development` / `Validate` / `Review` / `Done` (config-key names predate the 2026-05 Refine/Plan/Develop/Test rename and remain the canonical config API; the project-tether maps the new state slugs onto them — see `scripts/gh/lib/project-tether.mjs` `STATUS_CONFIG_KEYS`) | Kanban option IDs                            |
