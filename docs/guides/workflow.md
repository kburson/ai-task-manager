...
An audited false-delivery close is a separate human-only case. When a governed audit proves
that a historical root-epic no-commit record never delivered to trunk, first land either
that exact head or one protected-base integration head through a governed merged trunk PR.
An integration head must be an exact two-parent merge with the historical accepted SHA as
first parent and the incorporated trunk base as second parent; rebases, squashes, reversed
parents, and unrelated heads refuse. Then reopen and restore the issue
...
and Test, Review, intent, receipt, merged-PR, and trunk verification authority at the
delivery-head SHA. It writes immutable correction evidence naming both historical and
delivered SHAs before replacing the terminal marker at the delivered SHA and is not a
generic delivery backfill. See `rules/close.md` for the full retry and incompatibility
contract.
