...
- **`node_modules/` bootstrap (#791).** `node_modules/` is inherently untrackable, and every SessionStart/PreCompact/PostCompact hook plus the bash/agent/activity PreToolUse guards resolve through the _unscoped_ dogfood alias `node_modules/ai-task-manager` (a self-link to the repo root). A worktree where setup has not run lacks that link, so `node <missing path>` crashes and Claude Code treats the failed hook/guard as a non-blocking no-op — the timing roll-up, SessionStart self-heal, and every guard **silently fail open**. After creating a worktree, run `./scripts/dev-env/setup-local-worktree.sh`; it is the complete development-worktree setup and runs `npm ci`, then `npm run link:self`, then the verifier. `npm ci` alone does not repair the self-link. For a dependency-ready worktree with only a missing or foreign dogfood link, run `npm run link:self`. The explicit `link:self` script invokes `scripts/task-tracker/ensure-self-link.mjs`, which is idempotent and dogfooding-gated (`isDevPackage()` — no effect on consumer installs).
