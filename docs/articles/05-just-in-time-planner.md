...
[Series](README.md) | Previous: [The Backlog Becomes The Control Plane](04-backlog-as-control-plane.md) | Next: [Context Durability Is A Feature](06-context-durability.md)

## Draft Thesis

The most powerful part of AI Task Manager may not be the state machine or the timing log. It may be the planning discipline: keep large plans intentionally light, decompose them progressively, and perform detailed design only when the smallest executable work item is ready to be built.

That is just-in-time planning for agentic software delivery.

## Core Argument

Large product specifications have to describe intent, structure, and expected behavior. They should not pretend to freeze every implementation decision.
...
AITM applies that discipline to AI agents.

This is the same reason established SDLC patterns matter more with agent fleets. A human team can sometimes tolerate premature detail because people remember caveats and renegotiate assumptions. Independent agents need the structure to be explicit and current.

At the top of the backlog, detail stays deliberately thin:
...
The Plan state is where AITM spends detail.

At that point, the agent performs a deep-dive analysis against the codebase as it exists now, not as it existed when the original spec was written. The task definition is filled in with:
...
This is the last responsible moment for detailed planning. The agent has the product intent, the current repository state, and the immediate task boundary. That combination produces a better plan than either a giant up-front spec or a free-form coding prompt.

## Why This Matters For Agentic AI

AI agents are vulnerable to stale plans.
...
That is not failure. That is the system learning at the correct time.

## Discovered Work And Blocking Defects

Real implementation uncovers work that planning could not responsibly know.

When a task in Development discovers a blocking defect or necessary refactor, AITM can create a defect or refactor story, block the current task, switch to the discovered work, fix it in isolation, merge it back, then resume the original task against the updated codebase.

That matters because autonomous agents otherwise tend to hide discovered work inside the current branch. The result is scope creep, tangled commits, unclear review boundaries, and unreliable timing data.

AITM makes discovered work explicit:
...
The workflow preserves both momentum and auditability.

## AITM Perspective

AITM's JIT planner is a practical version of progressive elaboration for agent fleets.
...
This article explains how AITM manages planning depth. The next article, [Context Durability Is A Feature](06-context-durability.md), explains how AITM keeps the agent's process rules durable during long-running work.
