...
This plan delivers:
- A new `/task move <state>` verb as the **only** sanctioned state-change path for agents.
...
| Process-skip | State changed without `/task` event | Single chokepoint (`/task move`) + PreToolUse on direct `move-state.mjs` calls + PostToolUse auto-stamp safety net. |
...
| Drift (manual UI move) | Live state ≠ recorded | Pre-flight live-state read on every `/task move`; mismatch → refuse, instruct `/task reconcile`. |
...
Allowed transitions:
- **Forward, one step**: each adjacent pair above (left to right).
- **Backward, named only**: `validate → development` (test failure rework), `review → development` (review rework).

Anything else → refused with `reason: "illegal transition: <from> → <to>. Allowed: <next>."`.

### `/task move [<id>] <state>` semantics

Signature: `/task move [<id>] <state>`. `<id>` is optional and defaults to the currently-bound active issue (matches the pattern of `/task close [#N]`). `<id>` is **required** when no issue is active.

`/task move` is the only verb that mutates kanban state for agents. It:
...
4. Validates `lastKnownState → <state>` against the matrix; refuse if illegal.
5. Runs the gatekeeper for the target state (existing groom→analyze and analyze→development gates; new gates per state where missing).
6. Calls `move-state.mjs` internally with `AITM_INTERNAL=1` set.
7. Appends a `move:<state>` entry to the `⏱ Timing Log` comment with the **current** wall-clock timestamp.
8. Updates the `<!-- aitm-last-known-state: <state> -->` metadata line in the timing comment.
9. For `/task move done`: also performs every side effect of today's `/task close` (cascade close, fleet deregister, end-of-task counters). `/task close` becomes a deprecated thin alias.

Existing per-state verbs (`/task analyze`, `/task approve`, `/task review`, `/task close`) become thin aliases that delegate to `/task move <state>`. They remain for backward compatibility but emit a deprecation note.
...
Mechanism: PreToolUse hooks on `Edit`, `Write`, `NotebookEdit`, and `Bash` classify every operation into an **activity class**, look up the **cached current state** (from `.ai-task-manager/task-tracker-state.json`, written by every `/task move`), and refuse if the activity is not permitted in that state.
...
| Development | all |
| Validate | RUN_TESTS, RUN_BUILD, WRITE_ISSUE, READ_* (no WRITE_CODE — failed tests require `/task move <id> development` first) |
| Review | WRITE_ISSUE, WRITE_DOCS, READ_* (review-failure rework requires `/task move <id> development` first) |
| Done | READ_* only (issue closed; reopen and move-out required for further work) |
...
activity refused: <CLASS> on <target> is not permitted in state <STATE>.
  ...
  To proceed: <suggested action — e.g., "/task move <id> development", "/task start <issue#>", or "/task plan">
```
...
**False-positive remediation:** project edits `.ai-task-manager/activity-policy.json` to tighten or loosen globs. **No per-call override flag.** If a single edit is genuinely out of scope and the agent needs to do it, the answer is `/task move <id> development` — not a bypass.
...
`/task move` sets `AITM_INTERNAL=1` before invoking. Humans calling from a terminal still work (TTY check). PreToolUse on `Bash` is extended (in `bash-guard.mjs`) to refuse command lines matching `move-state\.(mjs|sh)` so agents can't bypass via shell.
...
5. **`/task move` is mandatory** — direct `move-state.mjs` calls forbidden.
...
- `scripts/task-tracker/state-machine.mjs` — transition matrix + validator (single source of truth, imported by `move-state.mjs` and `verbs/move.mjs`).
- `scripts/task-tracker/verbs/move.mjs` — `/task move <state>` implementation.
...
- `tests/move-verb.test.mjs` — gate firing, drift detection, retroactive-ts refusal.
...
- `scripts/task-tracker/task-tracker.mjs` — register `move` and `reconcile` verbs; convert `analyze`/`approve`/`review`/`close` into aliases that delegate to `move`.
...
- `skill/shared/SKILL.md` — document `/task move` and `/task reconcile`; mark `analyze`/`approve`/`review`/`close` as aliases.
...
- Existing gates in `verbs/analyze.mjs` and `verbs/approve.mjs` — invoked from `move.mjs` keyed on target state.
...
2c. **Activity-policy enforcement**
   - With active task in state Groom: attempt `Edit src/foo.ts`. Expect refusal naming state, class (`WRITE_CODE`), and remediation (`/task move <id> analyze`, then `analyze`→`development`).
   ...
   - With active task in state Validate: attempt `Edit src/foo.ts`. Expect refusal pointing to `/task move <id> development`.
...
3. **`/task move` happy path**
   - Create a test issue. Verify it starts in `backlog`.
   - Run `/task move groom`. Confirm: timing entry written with current ts, `lastKnownState` metadata updated, board moved, exit 0.
   - Run `/task move analyze`. Confirm groom→analyze gate (PR #49's 4-part check) fires; passes when checklist ticked, refuses when not.
   - Run `/task move done` and verify it performs every side effect today's `/task close` does (cascade close children, fleet deregister, final timing flush).

4. **`/task move` illegal transitions**
   - Issue in `backlog`. Run `/task move development`. Expect refusal: `illegal transition: backlog → development. Allowed: groom`.
   - Issue in `validate`. Run `/task move backlog`. Expect refusal.
   - Issue in `validate`. Run `/task move development`. Expect pass (legal backward).

5. **Drift detection**
   - Issue in `groom` (recorded). Manually move via GitHub UI to `analyze`.
   - Run `/task move development`. Expect refusal with drift message naming both states.
   ...
   - Run `/task move development`. Expect pass (gate permitting).

6. **Retroactive timestamp refusal**
   - Unit test invokes `buildRow` with a `ts` 5 minutes in the past. Expect throw.
   - Unit test invokes with `ts = Date.now()`. Expect success.

7. **Direct `move-state.mjs` block**
   - From an agent context (no `AITM_INTERNAL`), call `node scripts/gh/move-state.mjs <issue> done`. Expect refusal pointing to `/task move`.
