...
`migrateVcMirror` returns immediately when no live `npm run test:all` command exists. It therefore never inspects a matching tombstone or rewrites citations when the live command was removed by an earlier migration.
...
4. Resolve both replacement lane IDs from the live Verification Commands section. Replace the retired token using the existing exact-token helper without allocating or rewriting commands.
