...
**Status:** Draft for manual Codex-author/Claude-reviewer review. The user's instruction authorizes drafting from the agreed amended spec; it does not approve this replacement plan, hydrate issues, or authorize implementation. Preserve source-spec and historical review bytes, including their historical status wording.
...
| ---- | ----------------------- | -------------------------------------------------------------------------------- | ------------- |
| 1    | A1 / D characterization | Capture paired workflow baselines and prove operational presentation feasibility | None          |
| 2    | A1                      | Establish the complete shared decision and typed vocabulary contract             | 1 GO          |
...
Additional characterization owners are `scripts/maintenance/measure-guidance-candidate.mjs`, `scripts/tests/helpers/guidance-characterization.mjs`, and `scripts/tests/fixtures/1558/legacy-workflow/`. They are test/tooling additions, not runtime APIs. Task 1 pins a versioned standalone candidate serializer; Task 3 checks production projection against its fixtures and Task 14 replaces its response capture with actual CLI output.

## Implementation tasks

### Task 1: Capture paired workflow baselines and prove operational presentation feasibility

**User story:**

As a maintainer investing in context reduction,
I want reproducible costs and complete instruction/authority inventories before runtime changes,
So that extraction starts only when the amended presentation has a measured feasible path within the fixed budgets.

**Scope/files:** Create `scripts/maintenance/measure-guidance-candidate.mjs`, `scripts/tests/helpers/guidance-characterization.mjs`, and `scripts/tests/unit/task-tracker/lib/guidance-characterization.test.mjs`. Add the fixture artifacts listed below under `scripts/tests/fixtures/1558/`. Instrument existing executor read-only ports only through test harness injection; do not extract production evaluators, change runtime schemas/guards, slim skills, or add the operational loader in this child.

**Interfaces:** The maintenance tool supports `--all --json` for honest measurement, and `--all --assert-feasible --json` for the distinct foundation gate. Reports have a schema/version, source commit, source symbols, fixture digests, serializer version, scenario identity, and explicit candidate/actual classification. Task 2 consumes these artifacts, never a green unit test alone.

- [ ] **Step 1 — Inventory all seven actual paths.** Follow bind/resume/promote/test/review/deliver/close from dispatcher through delegates, guards, mutators and existing lanes, including evidence-v2. Record required and conditional reads, actual authority-resource IDs/subjects, source identities, applicability, policy enrichment, human decisions, warnings, refusal fan-out, locks, first effects, and retained instruction obligations. Classify transitive helpers as pure/read-only-network/ref-mutating/effectful. `commitsOnTrunkGate` → `fetchTrunk` is ref-mutating. Inventory every legacy refusal/warning by source symbol and expression fingerprint, not line number alone. Record all real guard registrations and known operational entrypoints; source inventory cannot be replaced by guard-slot counts.
- [ ] **Step 2 — Capture the current equivalent baseline before migration.** Freeze the loaded router, adapter, pickup and relevant skill Markdown bytes, plus command requests and complete stdout/stderr from the same deterministic lifecycle authority fixtures. Store a versioned runner and fixture data in `legacy-workflow/`; source commit pinning and byte digests make it reproducible after migration. Run each adapter. Capture transport requests including pages/retries separately from logical reads and local operations; existing code has no explanation CLI, so identify executor read-only phases and would-be query boundaries accurately. Measure controlled read-only live timing separately, with runtime/sample count and median/p95; do not perform live lifecycle mutations for a benchmark.
- [ ] **Step 3 — Add failing characterization tests and run VC1 to RED.** Negative cases remove a baseline command output, omit an action lane, use a guessed observation count, return an indeterminate result without causes, erase a warning/request/normalization, or count only selected JSON fields. Every case must fail completeness or preservation independently of its favorable size.
...
The helper implements a strict test-only schema/semantic oracle for §§13.2/15, not permissive hand-built successful outputs. Inputs include all arrays, typed arguments, and full digests where required; later production conformance compares against the frozen oracle cases.
...
- [ ] **Step 5 — Commit these independently inspectable artifacts:**
  - `action-observation-inventory.json`: all actions/lanes/resources/subjects/conditional reads and instruction/refusal/warning/human obligations, with source symbols and fingerprints.
  ...
  - `authority-baseline.json`: deterministic physical requests/pages/retries, logical reads, duplicate-read classification, baseline-pass/policy-enrichment lanes, and separately identified timing evidence.
  - `context-comparison.json` and ordered `legacy-workflow/` / `candidate-workflow/` transcripts: both adapter baselines, proposed static text and full traffic, category totals/delta, scenario/fixture/source/tool identities, and budget verdicts.
...
- [ ] **Step 6 — Run VC1 to GREEN, then run the separate foundation assertion.** GO requires full inventory/semantic fidelity, all representative working maxima, and measured lower candidate totals for both adapters under equivalent work. If it fails, commit the measured NO-GO as the characterization result, stop Tasks 2–17, and revise candidate presentation/instruction/query choices within the approved spec and fixed budgets. Reopen manual design/plan review only if the proposed remedy changes the contract. Do not relabel the old full-bundle probe as amended evidence.

**Acceptance criteria:**

- [ ] All seven actions and warning/human/refusal obligations have traceable inventories, deterministic fixtures, and preserved current-workflow runner/output evidence. <!-- aitm-verified vc-list="vc:1" -->
- [ ] Paired complete costs, sensitivity, reachable heavy case, timing provenance, and honest GO/NO-GO are reproducible; increased evidence-only cardinality does not enlarge routine output. <!-- aitm-verified vc-list="vc:1" -->
- [ ] The separate foundation assertion blocks all runtime work until feasible, semantically complete candidates meet fixed working maxima and reduce both adapter totals. <!-- aitm-verified vc-list="vc:1" -->
...
- [ ] `node --test scripts/tests/unit/task-tracker/lib/guidance-characterization.test.mjs && node scripts/maintenance/measure-guidance-candidate.mjs --all --json` <!-- id=1 -->
...
**Separate foundation gate (Task 1 completion record and mandatory prerequisite to Task 2):** `node scripts/maintenance/measure-guidance-candidate.mjs --all --assert-feasible --json`. An accurate NO-GO can pass VC1, but must fail this command. Record its exit status explicitly. VC17 is the final actual-evidence context/reduction gate, not a substitute for this early decision.
