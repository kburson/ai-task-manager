...
**Status:** Revised after replacement-plan round 1 (P1-01 through P1-04); awaiting manual reviewer confirmation. The round-1 author response is `docs/superpowers/reviews/1558/plan/2026-09-16-replacement-round-1-author-codex.md`. The user's instruction authorizes drafting from the agreed amended spec; it does not approve this replacement plan, hydrate issues, or authorize implementation. Preserve source-spec and historical review bytes, including their historical status wording.
...
**Interfaces:** The test-only `captureLegacyWorkflow({ sourceCommit, adapter, scenario, authorityFixture, scratchRoot })` runner returns ordered raw streams, loaded file bytes, simulated mutation/effect log and physical transport ledger. Freeze its implementation/source digests with baseline data. Task 1b consumes these artifacts; no feasibility verdict or second gate is introduced by this task.
...
- [ ] **Step 5 — Run VC1 to GREEN and commit.** All seven action lanes, effect/flag classifications, full traffic and known refusals must be covered before Task 1b starts. Review estimated remaining scope against the atomic threshold and split/pin the WBS before hydration if needed; this task does not bypass that rule merely because it is characterization.
...
- [ ] The test-only whole-lifecycle process harness exercises unchanged production predicates/formatting against isolated transports, with attempted-escape and package-exclusion proofs. <!-- aitm-verified vc-list="vc:1" -->
...
- [ ] **Step 1 — Verify Task 1a inputs.** Require its accepted complete action/effect/flag inventories, source and artifact digests, frozen loaded bytes, and real-command baseline capture for both adapters. Every mocked transport and excluded flag has an inventory row. Missing paths, uncaptured outputs, unclassified flag reads, bypass-derived success, or source drift block candidate certification.
