...
| `templates/plan-file.md`, `scripts/task-tracker/lib/plan-file.mjs`                                                     | Root/task intent in the live Markdown scaffold and retained unused compatibility export; unfinished scaffolds remain drafts.                   |
| `bin/lib/template-manifest.mjs`                                                                                        | Add defect and plan-file to the shared install/sync allowlist before asserting their mirrors.                                                  |
...
- Modify `bin/lib/template-manifest.mjs`: add `defect-body.md` and `plan-file.md` to `TEMPLATE_FILES` before synchronization or mirror assertions. Task 2 owns both additions; Task 5 consumes that manifest.
...
- Extend default-manifest sync and installer tests: `scripts/tests/unit/task-tracker/lib/sync-templates.test.mjs`, `scripts/tests/unit/task-tracker/lib/install.test.mjs`.
...
- [ ] **Step 3: Make non-stub creator/preflight story input optional.** Remove only `user-story-file` from required-flag lists. Do not place an undefined value into spawned preflight arguments. In preflight use the draft validation wrapper and substitute `''` on omission. In body verification validate the body-scoped draft form and compare the expected rendered story payload; preserve the first-H2 rule and every other required section, checkbox, hidden marker, field, and exact read-back invariant.
  ...
  Keep canonical template text sourced from author, rather than introducing new placeholder variants in template comments. Change that parameter documentation in epic, solo, and sub-issue templates from “Complete three-line” to “Empty, canonical template, or validated three-line User Story”. The defect template has no such parameter line. First extend `TEMPLATE_FILES` with defect and plan-file: sync iterates this allowlist, not the template directory. Then run `npm run sync:templates`, inspect generated changes, and run the mirror suite. This also makes `installTemplates()` install `plan-file.md` and manage `defect-body.md` in consumer projects on their next install/update; existing differing files follow its current `.bak` preservation behavior. Sync itself overwrites mirrors without that backup. Test default-manifest creation and refresh (not merely an injected `files` list), plus both installed artifacts and preservation behavior in the existing temporary installer fixture. Do not run an installer against a live consumer project.
...
node --test scripts/tests/unit/task-tracker/lib/sync-templates.test.mjs scripts/tests/unit/task-tracker/lib/install.test.mjs
...
- Modify plan scaffolds: `templates/plan-file.md`, `scripts/task-tracker/lib/plan-file.mjs` (retained, currently unused `PLAN_FILE_TEMPLATE` export).
- Manifest dependency: `bin/lib/template-manifest.mjs` (`TEMPLATE_FILES` gains `plan-file.md` and `defect-body.md` in Task 2); verify those additions before syncing the revised plan template with `npm run sync:templates`.
...
**Interfaces:** All provider adapter chains reach the same packaged shared rule through the router. The rule owns the rubric and examples; adapters link, rather than reproduce it. Provider-specific install recipes remain unchanged; the shared template manifest additions in Task 2 make the plan scaffold available to every installed provider.
...
- [ ] **Step 3: Route every affected workflow and remove contradictory mandates.** Add router rows for `user-story`/`story`, Plan work, and `split-plan`; have creation and approval/state-walk instructions load the shared rule. Update all required-fragment lists, defect wording, backlog examples, and blocker creation examples so `user-story.md` is optional during intake. Preserve required Scope, AC, and Story Origin fragments and all unrelated gates.
  ...
  Add the exact optional-input sentence used by the parity assertion to all three adapters. Route Claude, Codex, and Grok plan authoring through the shared rule, which points to `.ai-task-manager/templates/plan-file.md` in installed projects and the canonical `templates/plan-file.md` when working in the package repository. Replace Claude's sole direct discovery-scaffold pointer with this common guidance. Extend parity checks to follow that route for all three providers and verify the scaffold exists in the temporary installed fixture.
...
- [ ] **Step 4: Update the live scaffold and retained compatibility export.** `templates/plan-file.md` is the live scaffold referenced by Claude's discovery instructions. `PLAN_FILE_TEMPLATE` in `lib/plan-file.mjs` is an unused #414 export with no repository callers; it already differs from the Markdown template in blank-line formatting. Retain and update this export to avoid an unnecessary exported-API removal, while making no claim that discovery consumes it. Add the drift assertion rather than leave a second unchecked definition. Update both to include root `## Story Intent` and one `### Task 1:` skeleton with `#### Story Intent`, the four exact labels, and `**Verification Commands:**`. Keep unfinished values visibly incomplete and leave the verifier non-executable until replaced; the raw scaffold must remain a valid discovery draft but must refuse approval/splitting. Put a fully worked example in a fenced guide block, not a live extra task that could become a generic child. In `new-from-plan.test.mjs`, assert root/task structure for both templates, existing discovery compatibility, unfinished split refusal, and successful parsing/splitting after a fixture deliberately fills every field and supplies a real verifier. Compare the retained constant and canonical template after formatting-only normalization to catch drift. Task 2 must already have added both template names to `TEMPLATE_FILES`; after `npm run sync:templates`, require byte identity of the canonical and installed plan template.
...
- Modify: `docs/DESIGN.md` for story authoring and Plan approval behavior. `docs/QUICKSTART.md` is only a compatibility link page and needs no edit.
...
- [ ] **Step 4: Run targeted tests, then the issue's complete verification commands.** The issue root commands remain execution authority: `vc:1` quality/author/guard, `vc:2` approval, `vc:3` split, `vc:4` lifecycle, `vc:5` provider parity, `vc:6` fast tests, `vc:7` slow tests, `vc:8` lint, `vc:9` formatting, `vc:10` latest commit. Before Develop, use the sanctioned issue-body workflow to add `vc:6` to the AC2/AC3/AC4/AC6 citations (and AC1 for the expanded early registry tests) and append a root `vc:11` running the template, scaffold, directory-contract, issue-body-verifier, and creation-gate integration suites listed below. Add `vc:11` to affected AC1/AC2/AC5/AC6 citations. Preserve existing root command IDs and commands. `vc:6` runs the new unit files, including sync/installer coverage; `vc:7` runs only the slow lane. Under #1413, integration is CI-only in the local composite lanes: neither command includes it. It remains directly selectable, but this root command set runs integration files only through the explicit paths in `vc:4` and `vc:11`. Keep every Task 2 integration proof in `vc:11`; a local fast/slow pass cannot substitute for it. Run targeted new adapter/guard/corpus checks as additional evidence; do not claim those replace any root command. If dependencies prevent a test from loading, report that as unverified, not as a passing feature check.
...
node --test scripts/tests/integration/task-tracker/core/templates.test.mjs scripts/tests/integration/task-tracker/verbs/new-from-plan.test.mjs scripts/tests/integration/task-tracker/verbs/github-record-contract-writes.test.mjs scripts/tests/integration/task-tracker/lib/issue-body-verifier.test.mjs scripts/tests/integration/task-tracker/lib/create-issue-gate-compliance.test.mjs
npm test
npm run test:slow
npm run lint
