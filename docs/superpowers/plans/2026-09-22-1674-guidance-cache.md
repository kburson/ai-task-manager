...
- Source selection, package, validator, schema, vocabulary, and published-digest identity fields remain mandatory even on a content-hash fallback; inability to observe one refuses indeterminate.
...
- [ ] **Step 1: Write RED identity tests.** Test preserve-mtime replacement, chmod/ctime, missing stat field, linked `.git` file, selected index, split index, index tracking change, source adoption/deletion, profile/candidate collision, package/schema/parser/adapter/validator/registry/published-digest change, and forced refresh. Explicitly assert that chmod changes ctime-based identity and that changing the shared split-index file changes identity while the split index's own stat stays fixed. With device/inode unavailable, change registry and validator versions and assert a miss.
...
- [ ] **Step 3: Implement identity/observation minimally.** Use `statSync(..., { bigint: true })` where supported; compare the entire tuple, not individual optional fields. Resolve relative Git paths against the correct worktree Git directory, inspect split-index linkage, and preserve source/trust classification. Missing stat/index fields may substitute content digest plus fresh tracking checks only for those fields; all non-stat runtime and source-selection fields still have to match. If either part is unobservable, refuse indeterminate before effects.
...
- [ ] **Step 1: Add separate-process RED tests.** Compile in process A, start fresh processes with parser and validator traps for all four paths: manifest-only, agent, human, and invalid diagnostics. Each unchanged-source warm path has zero trap calls. Verify no human catalog load in agent mode and no diagnostic artifact open during invalid ordinary admission.
...
- [ ] **Step 2: Add corruption/concurrency RED tests.** Truncate, remove, schema-mutate, and digest-mutate each artifact; interleave two compilers with different source generations; interrupt before manifest rename. Force a source change between the first stat/read/stat, verify one retry, then change it again and assert a named indeterminate admission refusal with zero network, lock, session, guard, and provider effects. Readers must rebuild from the selected YAML on any artifact mismatch and never accept mixed content.
- [ ] **Step 3: Implement cold compile and warm load.** Stat/read/stat, retry one changed observation, then return a typed named indeterminate result that admission maps to no-effects refusal. Normalize and serialize deterministically, SHA-256 every artifact, write unique same-directory temporary files, rename artifacts, and rename manifest last. Re-read and verify schema/digest before accepting a warm artifact; on cross-generation mismatch retry from source. Memoize parsed agent index only within one process and identity.
...
- [ ] **Step 3: Measure all five §20.1 cases.** Record cold parse/validate/compile, warm manifest, warm agent, human, and invalid diagnostic loads from CI runner samples. Commit actual sample method, environment, medians/p95, and ceiling budgets with at least 20% unused headroom; CI remeasures and fails when live samples breach the working maxima. Assert relative cold/warm behavior and zero warm parser/validator invocations in fresh processes. Compare separately with #1673’s local B1 cold table, without treating that table as a CI budget or trusting a claimed `source: ci` field.
- [ ] **Step 4: Prove consumer release and commit.** Make `--assert-consumer-release`/`prepublishOnly` execute deterministic cross-process warm-load, invalidation, and production-only package safety checks on the package under test; they do not pass from the checked-in budget file or a caller certificate. CI additionally runs the five-case live timing/headroom job on its own runner. Run focused VC1, `npm run lint`, `npm run format:check`, `npm test`, `npm run test:integration`, `npm run test:slow`, `npm run lint:guidance-release-consumer`, and the production-only package test. Require positive B1+B2 certification and negative uncached/tampered fixtures. Commit `feat(guidance): certify cross-process B2 cache [#1674]`.
