...
For this plan, use a narrow append-only Git operation journal on the configured repository, at `refs/heads/aitm/delivery-waivers/<issue>/<deliveryOperationId>`. It is an adjacent consumption ledger, not an authorization source. Grants remain immutable GitHub workflow-exception comments. The journal contains only canonical bounded consumption/receipt-publication data and references to the existing grant; it contains no source tree, transcripts, credentials, or arbitrary files.
...
States are `burned -> receipt-requesting -> completed`. `burned` is the immutable authorization/consumption event. The `receipt-requesting` transition pins the exact receipt bytes and a process-run nonce before any comment POST; only the invocation that successfully performs that transition may POST once. Later invocations may reconcile that pending request, but cannot POST it again based solely on an empty comment listing. A completed write with a lost response is recovered by exact readback. An unresolved request remains `indeterminate`, with no automatic timeout takeover or replacement operation. This deliberately preserves uncertainty instead of risking two terminal receipts.
...
// journal.read() -> null | { oid, state, burn, publication }
...
**Interfaces:** Implement Shared Interfaces' journal, burn, and publication APIs. Use schema `aitm.delivery-waiver-consumption/v1` for the immutable burn and `aitm.delivery-waiver-journal-entry/v1` for transitions. Burn keys are exactly `schema`, `repository`, `issue`, `deliveryOperationId`, `waiverRecordId`, `waiverRevision`, `waiverScopeDigest`, `waiverReasonDigest`, `acceptedHeadSha`, `intentId`, `mergeCommitSha`, `authorizedAt`, and `grantDigest`. The canonical operation key excludes grant revision. Journal entries contain exactly `schema`, `sequence`, `predecessorOid`, `state`, `burn`, and `publication`; publication is null until requesting, then includes `receiptBody`, `receiptDigest`, `runId`, and nullable `commentNodeId`.
...
assert.equal((await hostA.read()).state, 'burned');
```

`hostA` and `hostB` use separate journal adapters against the same disposable bare remote; candidate objects have the exact burn keys above and different valid intent IDs.
...
```js
const ref = `refs/heads/aitm/delivery-waivers/${issue}/${deliveryOperationId}`;
const args = [
  'push',
  '--porcelain',
  '--no-follow-tags',
...
- [ ] Re-read remote authority after every push, including transport errors. Exact confirmed state permits continuation; differing intent/digest refuses; unknown outcome remains indeterminate. No unrelated ref is updated and no ref is deleted. The journal never grants a waiver: only Task 5's validated grant plus fresh complete verifier evidence can construct the initial burn.
...
- [ ] Use two clones and separate processes to prove one burn winner and one receipt POST winner. Insert crash checkpoints before push, after remote acceptance, before POST, after POST, and before completed-state write. Simulate branch-write refusal and remote identity mismatch; both fail closed. Test no-grant preparation and read-only preflight perform zero pushes. Commit as `feat: serialize delivery waiver consumption durably [#1787]`.
...
- [ ] Require unique operation-to-intent ownership in projection. Test unknown/mixed version pairs, extra/missing keys, wrong original intent, altered reason/scope/grant/burn digest, excessive comment size, duplicate receipts, and same operation with another intent. Re-run every legacy record fixture, then commit as `feat: represent waived delivery with pinned evidence [#1787]`.
...
- [ ] Dispatch from verified durable state: existing v4 receipt -> pinned verification/idempotent return; exact confirmed burn -> same-intent pinned retry; otherwise -> fresh current authority. Build/read back a v3 intent before burn, retaining the original intent identity/timestamp. Re-read issue, PR, grants, and complete live verifier evidence at the consumption boundary. Place burn after the last non-waived verification, including the existing late reachability check.
...
- [ ] Test race injections after preparation, after intent readback, before burn, and during receipt publication. Include revoked/expired grant before burn, fresh human approval after re-scoping, and confirmed-burn retry after expiry. Re-run existing deliver-close, historical/external recovery, and #1755 integration suites. Commit as `feat: consume delivery waivers at the receipt boundary [#1787]`.
