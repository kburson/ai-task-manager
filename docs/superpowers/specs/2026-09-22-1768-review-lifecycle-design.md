...
6. The parent epic holds the master-plan reference and hydration receipt.
   Children retain their native parent link and standalone issue content. No new
   per-child plan-version, commit, or task-identifier fields are required.
...
the plan version, then review it against that spec revision. Once implementation
has begun, version references remain historical provenance, not automatic
invalidation of already hydrated children. Normal issue changes and subsequent
planning decisions can address discoveries without a forced state rewind.
...
pauses are excluded from engaged time. The resulting spec receives its normal
...
1. Create one governed Backlog stub through the sanctioned issue creator and
   durably associate its number with the operation ID.
...
The existing title-only `/task new "title"` path stays supported. With an
...
Backlog creation does not start hosted review. When the issue enters Refine,
...
After SAR, AITM evaluates risk from concrete signals: security or privacy
consequences, irreversible data or migration actions, externally consumed
interfaces, multiple subsystems/providers, unresolved assumptions, and severe
or repeated SAR findings. File length may affect review cost but never decides
the level alone. The rubric and its thresholds are versioned and deterministic.
...
Refine-to-Ready-for-Planning requires a terminal accepted result for the
selected path, a current spec reference, and the existing AITM refinement
...
On pickup from Ready for Planning, a solo issue or epic enters Plan. A
...
priority, size, estimate, and normal parent link. An aggregate hydration SAR
...
child inventory at that moment; it does not create a continuing per-child plan
lock.
...
findings prompt revision and another round. Child-specific discoveries may
...
diffs, exact-byte acceptance, and an auditable terminal result for spec, plan,
hydration, and Deep-Dive reviews. It does not clone the external package's
...
3. A Backlog stub needs no hosted review; Refine-to-R4P requires the accepted
   spec review selected by the capability-aware risk rubric.
