...
// every such file. See lib/lint-test-coverage-reach.mjs for the detector and
...
import { findNoReachTestFiles } from '../task-tracker/lib/lint-test-coverage-reach.mjs';
...
const testFiles = execSync(`find ${ROOT} -type f -name '*.test.mjs'`, { encoding: 'utf8' })
  .trim()
  .split('\n')
  .filter(Boolean);
