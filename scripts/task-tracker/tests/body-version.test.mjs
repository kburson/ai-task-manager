...
test('parseBodyVersion: present marker → integer N', () => {
  const body = 'body\n\n<!-- aitm-body-version: 7 -->\n';
  assert.equal(parseBodyVersion(body), 7);
});

test('parseBodyVersion: tolerates whitespace variants', () => {
...
test('stampBodyVersion: appends marker at tail when absent', () => {
  const out = stampBodyVersion('# title\n\nbody', 1);
  assert.match(out, BODY_VERSION_MARKER_RE);
  assert.equal(parseBodyVersion(out), 1);
  // Should end with the marker on its own line.
  assert.ok(out.trimEnd().endsWith('<!-- aitm-body-version: 1 -->'));
});

test('stampBodyVersion: appends with one leading blank line for separation', () => {
  const out = stampBodyVersion('body', 3);
  assert.ok(out.includes('body\n\n<!-- aitm-body-version: 3 -->'));
});

test('stampBodyVersion: replaces marker in place, preserves surrounding text', () => {
  const before = 'head\n\n<!-- aitm-body-version: 4 -->\n\ntail';
