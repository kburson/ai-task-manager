...
function setupSandbox() {
  ...
  writeFileSync(
    ...
    JSON.stringify({
      ...
      kanbanOptionGroom: 'OPT_g',
      kanbanOptionAnalyze: 'OPT_a',
      kanbanOptionDevelopment: 'OPT_d',
      kanbanOptionValidate: 'OPT_v',
...
async function runNode(script, args, { sandbox, binDir, env = {}, expectExit = 0 } = {}) {
  ...
  assert.equal(code, expectExit, `expected exit ${expectExit}, got ${code}. stderr:\n${result.stderr || ''}\nstdout:\n${result.stdout || ''}`);
...
try {
  ...
  {
    const sandbox = setupSandbox(); cleanup(sandbox);
  ...
  // 5. Dirty close, --answer cancel → abort (exit 0), no close
  {
    ...
    const r = await runNode(TT, ['close', '#202', '--answer', 'cancel'],
      { sandbox, binDir });
  ...
  // 6. Headless dirty close (CI=1, no --answer) → exit 5
  {
    ...
    const r = await runNode(TT, ['close', '#203'],
      { sandbox, binDir, env: { CI: '1' }, expectExit: 5 });
...
`);
    ...
    writeFileSync(
      ...
      JSON.stringify({ '#206': { worktreePath: altWorktree, branch: 'x', startedAt: new Date().toISOString(), status: 'active' } })
    );
  ...
  // 10. TT_SKIP_DIRTY_CHECK=1 disables the gate even with dirty workspace
  {
    ...
    const r = await runNode(MOVE, ['207', 'review'],
      { sandbox, binDir, env: { TT_SKIP_DIRTY_CHECK: '1' } });
    assert.doesNotMatch(r.stderr, /Workspace is dirty/);
  }
