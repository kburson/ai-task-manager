...
assert.match(
  help,
  /plan[^\n]*Sprint-Planning/i,
  'plan description must mention Sprint-Planning'
);
assert.match(
  help,
  /Refine→Plan/,
  'plan description must show the refine→plan transition'
);
...
const discoverBlock = help.match(/\/task discover[\s\S]*?(?=\n\s{0,2}\/task|\nState transitions|$)/);
...
console.log('PASS: help text distinguishes /task discover (backlog gen) from /task plan (Sprint-Planning)');
