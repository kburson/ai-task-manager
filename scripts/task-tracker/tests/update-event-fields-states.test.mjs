...
function makeTempEnv(ghScript) {
  const temp = mkdtempSync('/private/tmp/aitm-update-event-states-');
...
for (const state of states) {
  ...
  // refine/plan/test/review/done have empty bindings in the default config,
  // so no "set for #999" output is expected; develop should write startTime.
  if (state === 'develop') {
    assert.match(
      result.stdout,
      /startTime set for #999/,
      `develop should emit startTime set\nstdout: ${result.stdout}`
    );
  } else {
    assert.ok(
