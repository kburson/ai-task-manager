...
describe('unit runner bounded sections (#1157)', () => {
  it('passes individually bounded phases when their combined duration exceeds the ceiling', () => {
    const result = evaluateSections({
      lane: 'unit',
      env: {},
      sections: [section('pooled', 400_000, 698), section('serial', 300_000, 92)],
    });

    assert.equal(result.totalElapsedMs, 700_000);
    ...
    assert.deepEqual(
      ...
      [
        { name: 'pooled', count: 698, breached: false },
        { name: 'serial', count: 92, breached: false },
      ]
    );
  });

  for (const name of ['pooled', 'serial']) {
    it(`fails closed and names an over-budget ${name} phase`, () => {
      const other = name === 'pooled' ? 'serial' : 'pooled';
      const result = evaluateSections({
        lane: 'unit',
        env: {},
        sections: [
          section(name, DEFAULT_SECTION_CEILING_MS + 1),
          section(other, DEFAULT_SECTION_CEILING_MS - 1),
        ],
      });
  ...
  it('omits empty phases so single-phase lanes retain one fail-closed ceiling', () => {
    const result = evaluateSections({
      lane: 'slow',
      env: {},
      sections: [section('pooled', 0, 0), section('serial', DEFAULT_SECTION_CEILING_MS + 1, 50)],
    });
  ...
  it('keeps every internal all-lane phase exempt', () => {
    const result = evaluateSections({
      lane: 'all',
      env: {},
      sections: [section('pooled', 3_600_000), section('serial', 3_600_000)],
    });
  ...
  it('reports both phase timings and the observational aggregate', () => {
    ...
    assert.match(report, /pooled=400\.0s\/600s \(698 files\)/);
    assert.match(report, /serial=300\.0s\/600s \(92 files\)/);
    assert.match(report, /aggregate=700\.0s/);
  });
});
