#!/usr/bin/env node
// @story #865
// The `docs` issue kind: a commit-bearing kind whose functional-test DoD item is
// diff-conditional — "the kind declares, the diff decides." A docs-kind issue
// drops the `tests` item (and its derived `npm run test:all` verification
// command) ONLY when the actual `trunk...HEAD` diff is provably
// documentation-only; default-deny keeps the suite for any unclassified path.
//
// This file is the single verifier bound to all eight ACs of #865:
//   AC1  `docs` ∈ VALID_KINDS and ∉ NO_COMMIT_KINDS
//   AC2  `/task kind <N> docs` marker round-trips through parseIssueKind
//   AC3  docs kind + docs-only diff drops the tests item AND its test:all VC
//   AC4  docs kind + any non-doc path still requires the tests item
//   AC5  unrecognized path counts as functional (default-deny), unknown extension
//   AC6  lint + format DoD items remain required for docs kind
//   AC7  commit-trace attributes a docs issue like code (not a no-commit kind)
//   AC8  workflow.md documents the docs kind and the diff-decides rule
...
function makeFixture() {
  const dir = mkdtempSync(path.join(projectScratchDir('test'), 'docs-kind-dod-'));
...
// ---------------------------------------------------------------------------
// AC1 — `docs` is a member of VALID_KINDS and NOT NO_COMMIT_KINDS
// ---------------------------------------------------------------------------
describe('AC1: docs is a commit-bearing valid kind', () => {
  it('docs ∈ VALID_KINDS', () => {
    assert.ok(VALID_KINDS.has('docs'), 'docs must be a valid kind');
  });
  it('docs ∉ NO_COMMIT_KINDS', () => {
    assert.ok(!NO_COMMIT_KINDS.has('docs'), 'docs must NOT be a no-commit kind');
  });
...
// ---------------------------------------------------------------------------
// AC2 — the kind marker round-trips (what `/task kind <N> docs` writes/reads)
// ---------------------------------------------------------------------------
describe('AC2: docs kind marker round-trips', () => {
  it('normalizeKind accepts docs (case/space insensitive)', () => {
    assert.equal(normalizeKind('docs'), 'docs');
    assert.equal(normalizeKind('  DOCS '), 'docs');
  });
  it('setIssueKindMarker → parseIssueKind round-trips docs', () => {
    const body = 'Body.\n\n## AITM Progress Markers\n';
    const marked = setIssueKindMarker(body, 'docs');
    assert.match(marked, /aitm-issue-kind kind="docs"/);
    assert.equal(parseIssueKind(marked), 'docs');
  });
...
describe('diff classifier: isDocPath / diffIsDocsOnly / docsKindDropsTests', () => {
  ...
  it('docsKindDropsTests: only docs kind + provably docs-only diff', () => {
    assert.equal(docsKindDropsTests('docs', ['docs/a.md']), true);
    assert.equal(docsKindDropsTests('docs', ['scripts/x.mjs']), false);
    assert.equal(docsKindDropsTests('docs', []), false);
...
describe('AC3: docs-only diff drops the functional tests item', () => {
  it('filterDodForKindAndDiff drops dod:functional:tests for docs-only diff', () => {
    const filtered = filterDodForKindAndDiff(RUNTIME_DOD_TEXT, 'docs', [
  ...
  it('preflight --kind docs with a docs-only changed-paths-file omits tests + test:all VC', async () => {
    const fx = makeFixture();
    try {
      const cp = writeChangedPaths(fx, ['docs/guides/workflow.md', 'README.md']);
      const r = await runPreflight(
        subIssueArgs(fx, ['--kind', 'docs', '--changed-paths-file', cp])
      );
...
// AC4 — docs kind + any non-doc path still requires the tests item
// ---------------------------------------------------------------------------
describe('AC4: a mixed (non-doc) diff keeps the tests item for docs kind', () => {
  ...
  it('preflight --kind docs with a functional path keeps tests + test:all VC', async () => {
    const fx = makeFixture();
    try {
  ...
  it('docs kind WITHOUT a changed-paths-file keeps tests (safe default)', async () => {
    const fx = makeFixture();
    try {
      const r = await runPreflight(subIssueArgs(fx, ['--kind', 'docs']));
      assert.equal(r.code, 0, r.stderr);
      assert.match(r.stdout, /dod:functional:tests/);
      assert.match(r.stdout, /- \[ \] `npm run test:all`/);
    } finally {
...
describe('AC5: an unknown extension counts as functional (default-deny)', () => {
  it('filterDodForKindAndDiff keeps tests for an unknown extension', () => {
    const filtered = filterDodForKindAndDiff(RUNTIME_DOD_TEXT, 'docs', ['release-notes.rst']);
    assert.match(filtered, /dod:functional:tests/);
  });

  it('preflight --kind docs with only an unknown-extension path keeps tests', async () => {
    const fx = makeFixture();
    try {
...
// AC6 — lint + format DoD items remain required for docs kind
// ---------------------------------------------------------------------------
describe('AC6: lint/format items survive for docs kind', () => {
  it('the lint+format line survives even when tests is dropped', () => {
    const filtered = filterDodForKindAndDiff(RUNTIME_DOD_TEXT, 'docs', ['docs/a.md']);
...
// ---------------------------------------------------------------------------
// AC7 — commit-trace attributes a docs issue exactly as for code: docs is a
// commit-bearing kind, so isNoCommitKind is false and the develop→test gate
// keeps the `### 🔗 Commits` trail requirement (never the deliverable lane).
// ---------------------------------------------------------------------------
describe('AC7: docs is commit-bearing (attributed like code)', () => {
  it('isNoCommitKind is false for a docs-marked body', () => {
...
// AC8 — workflow.md documents the docs kind and the diff-decides rule
// ---------------------------------------------------------------------------
describe('AC8: workflow.md documents docs kind + diff-decides rule', () => {
  it('names the docs kind, the #865 story, and the decide-by-diff principle', () => {
    const doc = readFileSync(WORKFLOW_DOC, 'utf8');
    assert.match(doc, /docs.*kind/i);
...
// every non-docs kind and for a docs kind with no/empty diff.
// ---------------------------------------------------------------------------
describe('back-compat: no conditional drop off the docs-only path', () => {
  ...
  it('docs kind with a null/empty diff equals filterDodForKind (keeps tests)', () => {
    assert.equal(
      filterDodForKindAndDiff(RUNTIME_DOD_TEXT, 'docs', null),
      filterDodForKind(RUNTIME_DOD_TEXT, 'docs')
    );
    assert.match(filterDodForKindAndDiff(RUNTIME_DOD_TEXT, 'docs', []), /dod:functional:tests/);
  });
});
