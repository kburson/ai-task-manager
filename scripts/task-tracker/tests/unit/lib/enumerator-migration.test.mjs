...
import { discoverFiles, discoverTestFiles } from '../../../lib/discover-test-files.mjs';
import { unitTestPath, coLocatedTestPath, findUnitTests } from '../../../find-unit-tests.mjs';
...
const CONSUMERS = [
  {
    rel: 'scripts/task-tracker/tests/audit-story-tags.mjs',
  ...
  {
    rel: 'scripts/task-tracker/tests/audit-line-cap.mjs',
    importsFrom: /discover-test-files\.mjs/,
  },
  {
...
// --- AC3 — find-unit-tests reconciled: co-located tests are found -------------

test('AC3: findUnitTests resolves co-located and conventional unit tests', () => {
  // Pure mappers.
  assert.equal(unitTestPath('scripts/foo/bar.mjs'), 'scripts/task-tracker/tests/unit/bar.test.mjs');
  ...
  // Against a synthetic discovered set: a source with a co-located test resolves
  // to it; a source with only a conventional test resolves to that; a source
  // with neither resolves to nothing.
  const discovered = [
    'scripts/gh/create-issue.test.mjs', // co-located
    'scripts/task-tracker/tests/unit/widget.test.mjs', // conventional
  ];
  ...
  assert.deepEqual(got, [
    ...
    'scripts/task-tracker/tests/unit/widget.test.mjs',
  ]);

  // Live: the real create-issue co-located test is discoverable and the mapper
  // finds it (it was invisible to the single-directory mapper before #875).
  const live = findUnitTests(['scripts/gh/create-issue.mjs'], opts);
  assert.deepEqual(live, ['scripts/gh/create-issue.test.mjs']);
});
