...
export async function confirmBlastRadius({
  ...
  log(`About to write ${count} issue(s)${count ? `: ${list}` : ''}\n`);
  ...
  if (!isTTY) {
    warn(
      `refusing: ${count} issue(s) exceed the confirmation threshold (${threshold}) and ` +
  ...
  const answer = await ask(`Write ${count} issues? [y/N] `);
