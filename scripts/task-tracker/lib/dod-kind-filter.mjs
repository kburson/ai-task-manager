...
// ── #865: docs-only kind — "the kind declares, the diff decides" ────────────
//
// A `docs-only` kind must NOT statically exclude the `tests` DoD item the way
// `spike`/`research` do: those kinds never carry code, but a `docs-only` issue
// can quietly edit `scripts/**` and would then launder its way out of the suite
// by annotation alone. So the relief is conditional on the ACTUAL changed-path
// set of the issue's `trunk...HEAD` diff, classified here under a DEFAULT-DENY
// rule: only a provably documentation-only diff drops the item; anything
// unclassified counts as functional and keeps it.
//
// This is the unification of #865's diff-conditional relief with trunk #923's
// `docs-only` kind name (#859 rework): the kind is spelled `docs-only`, and the
// test machinery drops only when the diff earns it, never by label alone.
...
/**
 * True when a `docs-only`-kind issue may drop the functional-test DoD item for
 * this diff: `kind === 'docs-only'` AND the diff is provably documentation-only.
 * Any other kind, or any non-doc / unclassified / empty diff, returns false.
 */
export function docsKindDropsTests(kind, changedPaths) {
  return String(kind || '').toLowerCase() === 'docs-only' && diffIsDocsOnly(changedPaths);
}
 ...
 *
 * For every non-`docs-only` kind, and for any `docs-only` diff that is not
 * provably docs-only, the extra drop never fires and the output is exactly
