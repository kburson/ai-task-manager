...
import { hasNestedMetadataHeading, substantiveMetadataFields } from './metadata-section.mjs';
...
function hasPlanningField(body) {
  for (const field of substantiveMetadataFields(body, PLAN_METADATA_HEADING)) {
    if (!PROVENANCE_KEYS.has(field.key.toLowerCase())) return true;
  }
  return false;
}
