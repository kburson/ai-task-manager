...
export function codexBootstrapBlock({ scope = 'repo' } = {}) {
  ...
  return [
    ...
    '',
    '- `/task promote #N` — advance one state along Backlog → Refine → Plan → Develop → Test → Review → Done',
    '- `/task approve #N` — records human review-approval marker on a Review-state issue (gate for `close`)',
    '- `/task review #N` — Develop → Test (agent reports test outcomes; gate before close)',
