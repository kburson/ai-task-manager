...
async function measure(id, state, collect, values, route = 'collector') {
  const fixture = attemptFor({ actionId: id, state, values });
  const result =
    route === 'explanation-evaluator'
      ? await evaluateAction({
          ...
          inputs: {
            ...
            sessionState: { active: null, paused: id === 'resume', lastActive: ISSUE },
            config: { repo: REPOSITORY, preferences: { gateAssigneeMatch: false } },
          },
          attempt: fixture.attempt,
          deps: { effectAttempts: () => [], collectReadiness: () => collect(fixture) },
        })
...
/** Run shipped read-only collectors directly or through the Explain evaluator. */
export async function measureFixedExplanationAuthorityReads() {
  const route = 'explanation-evaluator';
  const measureRoute = (id, state, collect, values) => measure(id, state, collect, values, route);
  return Object.freeze({
    method: `createObservationAttempt-read-port-ledger/v1:${route}`,
    actions: await Promise.all([
      ...
      measureRoute('promote', 'refine', ({ attempt, scope, issueBody }) =>
      ...
      measureRoute('test', 'test', ({ attempt, scope, issueBody }) =>
      ...
      measureRoute('review', 'test', ({ attempt, scope, issueBody }) =>
