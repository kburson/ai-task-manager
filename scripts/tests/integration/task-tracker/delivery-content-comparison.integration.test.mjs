...
function repo(t) {
  ...
  git('merge', '-q', '--no-ff', 'source', '-m', '[#1811] observed merge');
  const merged = git('rev-parse', 'HEAD');
  git('switch', '-q', '-c', 'squashed', integrationBase);
  git('merge', '-q', '--squash', 'source');
  git('commit', '-qm', '[#1811] observed squash');
...
test('manual merge, squash, and rebase record observed method for either proposed preference', async (t) => {
  const f = repo(t);
  const deps = createDefaultDeliverDeps({ projectDir: f.cwd, cfg: { repo: 'owner/repo' } });
  const evidence = [f.sourceFirst, f.sourceHead].map((oid) => ({
    oid,
    parents: [f.git('rev-parse', `${oid}^`)],
    tree: f.git('rev-parse', `${oid}^{tree}`),
    message: f.git('show', '-s', '--format=%B', oid),
  }));
  for (const [observedMethod, mergeCommitSha] of [
    ['merge', f.merged],
    ['squash', f.squashed],
    ['rebase', f.replayHead],
  ]) {
    for (const proposedMethod of ['merge', 'squash']) {
      const intent = buildDeliveryIntent({
        ...
        baseRef: 'main',
        headRef: 'source',
        expectedHeadSha: f.sourceHead,
        mergeMethod: proposedMethod,
        attributionTokens: ['#1811'],
        commitTitle: '[#1811] Governed PR delivery',
        commitMessage: `PR #1812\nSource: ${f.sourceHead}\n\nAttribution: [#1811]`,
        provider: 'codex',
        sessionId: 'session-1811',
        clientCreatedAt: '2026-09-23T00:00:00.000Z',
      });
      const pullRequest = {
        number: 1812,
        state: 'MERGED',
        ...
        headRefOid: f.sourceHead,
        mergeCommitSha,
        mergeMethod: observedMethod,
        mergedAt: '2026-09-24T00:00:00.000Z',
        headRefDeleted: false,
        sourceCommitsComplete: true,
        sourceCommitsHeadSha: f.sourceHead,
        sourceCommits: evidence.map(({ oid, message }) => ({
          oid,
      ...
      const verified = await verifyDeliveredPullRequest({
        acceptedSha: f.sourceHead,
        acceptedReviewSha: f.sourceHead,
        intent,
        intentCreatedAt: '2026-09-23T00:00:00.000Z',
        localHeadSha: f.sourceHead,
        pullRequest,
        recovery: false,
        testReceiptSha: f.sourceHead,
        fetchOriginTrunk: async () => {},
        isAncestor: async ({ ancestor, descendant }) =>
          descendant === 'origin/main'
            ? ancestor === mergeCommitSha
            : deps.isAncestor({ ancestor, descendant }),
        inspectMergeCommit: deps.inspectMergeCommit,
        compareDeliveryContent: deps.compareDeliveryContent,
        resolveTrunkHeadSha: async () => mergeCommitSha,
        attributingCommits: async () => [{ sha: mergeCommitSha, subject: '[#1811] integrated' }],
      });
      const receipt = buildDeliveryReceipt(verified.receiptInput);
      assert.equal(receipt.schema, 'aitm.delivery-receipt/v5');
      assert.equal(receipt.mergeMethod, observedMethod);
      assert.equal(receipt.observedIntegration.method, observedMethod);
