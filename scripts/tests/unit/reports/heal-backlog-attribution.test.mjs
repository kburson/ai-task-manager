...
import {
  ...
  buildTrunkShaSet,
  LOG_FIELD_SEP,
} from '../../../reports/lib/attribution-resolver.mjs';
import { rewriteCommitsMarker } from '../../../reports/heal-backlog-attribution.mjs';
...
test('apply off by default: the mutation is guarded and the rewrite primitive is pure', () => {
  ...
  const src = readFileSync(path.join(__dir, '../../../reports/heal-backlog-attribution.mjs'), 'utf8');
...
test('value-report consumes resolver: admits attributed dataless issues, drops dead', () => {
  ...
  const src = readFileSync(path.join(__dir, '../../../reports/generate-value-report.mjs'), 'utf8');
