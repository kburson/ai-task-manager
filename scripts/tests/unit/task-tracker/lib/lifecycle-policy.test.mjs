...
test('lifecycle policy has a one-way, side-effect-free import boundary', async () => {
  const packageFiles = [
    '../../../../task-tracker/lib/lifecycle-policy/states.mjs',
    '../../../../task-tracker/lib/lifecycle-policy/executable-transitions.mjs',
    '../../../../task-tracker/lib/lifecycle-policy/index.mjs',
  ];
...
test('operational registries derive topology and config metadata from lifecycle policy', async () => {
  const movePolicySource = await readFile(
    fileURLToPath(new URL('../../../../task-tracker/lib/move-state/policy.mjs', import.meta.url)),
    'utf8'
  );
  const stateObjectsSource = await readFile(
    fileURLToPath(new URL('../../../../task-tracker/states/index.mjs', import.meta.url)),
    'utf8'
  );
