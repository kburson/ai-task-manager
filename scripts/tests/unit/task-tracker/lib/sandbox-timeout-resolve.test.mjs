...
test('AC5: verbs/test.mjs no longer defines its own SANDBOX_TIMEOUT_MS', () => {
  const src = readFileSync(path.resolve(__dir, '../../../task-tracker/verbs/test.mjs'), 'utf8');
  assert.ok(
...
test('AC5: the stale "~100 files" comment is gone from the constant', () => {
  const timeoutsSrc = readFileSync(
    path.resolve(__dir, '../../../task-tracker/lib/process-timeouts.mjs'),
    'utf8'
  );
  const testSrc = readFileSync(path.resolve(__dir, '../../../task-tracker/verbs/test.mjs'), 'utf8');
  assert.ok(
