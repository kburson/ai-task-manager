...
import { classifyEvent } from '../../../../task-tracker/lib/bind-event.mjs';
import {
  buildRow,
...
const LIFECYCLE_EVENTS = [
  ...
  ['ready-for-plan:started', 'lifecycle', 'ready-for-plan'],
...
test('legacy aliases remain readable while retired vocabulary cannot be emitted', () => {
  ...
  assert.equal(stageOfTimingEvent('assigned:started'), 'ready-for-plan');
  assert.equal(stageOfTimingEvent('on-deck:started'), 'ready-for-plan');
...
test('timing-event policy is a leaf package with only sibling lifecycle identity input', () => {
  for (const file of ['catalog.mjs', 'parameterized.mjs', 'legacy.mjs', 'index.mjs']) {
    const source = readFileSync(
      new URL(`../../../../task-tracker/lib/timing-events/${file}`, import.meta.url),
      'utf8'
    );
