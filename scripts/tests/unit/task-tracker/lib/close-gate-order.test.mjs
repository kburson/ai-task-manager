...
test('source: forced pre-move and final move receive resolved review authority', () => {
  ...
  assert.match(forceBlock, /reviewAuthority:\s*terminalReviewAuthority\(\)/);
  ...
  assert.match(finalBlock, /reviewAuthority:\s*terminalReviewAuthority\(\)/);
});
