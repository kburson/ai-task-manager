...
// direct backlog-to-refine boundary in #1211).
//
...
// is outside {refine, ready-for-plan, plan, develop}.
...
import {
  ...
  GUARD_ID,
} from '../../../../task-tracker/lib/backlog-exit-child-parent-state-guard.mjs';
...
for (const parentState of ['backlog', 'test', 'review', 'done']) {
  test(`refuses when parent state is "${parentState}"`, async () => {
    ...
    assert.match(
      result.reason,
      new RegExp(
        `parent #100 is in ${parentState}; child cannot enter refine until parent is in refine, ready-for-plan, plan, or develop`
      )
    );
