...
import { runEvidenceMarkers } from '../../../../task-tracker/verbs/evidence-markers.mjs';
...

{
  const body = [
    ...
    '- [ ] `node scripts/tests/unit/task-tracker/existing.test.mjs`',
    '',
    metadata,
  ].join('\n');
...

{
  const body = [
    '## Acceptance Criteria',
    '- [ ] Covered AC <!-- aitm-verified cmd="`node scripts/tests/unit/task-tracker/new.test.mjs`" -->',
  ...
  assert.deepEqual(audit.missingVerificationCommands, [
    'node scripts/tests/unit/task-tracker/new.test.mjs',
    'npm run lint',
  ]);
...

{
  ...
  const result = buildEvidenceBackfill(body, {
    mappings: {
      'Plain AC': ['node scripts/tests/unit/task-tracker/evidence-marker-backfill.test.mjs'],
    },
  });
  ...
  assert.match(
    result.body,
    /### Verification Commands\n\n- \[ \] `node scripts\/tests\/unit\/task-tracker\/evidence-marker-backfill\.test\.mjs`/
  );
...
// section and audit kept reporting them as `missing-verification-command`.
{
  ...
  const result = buildEvidenceBackfill(body, {
    mappings: {
      'Plain AC': ['node scripts/tests/unit/task-tracker/regression-new.test.mjs'],
    },
  });
  assert.equal(result.ok, true, JSON.stringify(result));
  const lines = result.body.split('\n');
  const newCmdIdx = lines.findIndex((l) =>
    l.includes('node scripts/tests/unit/task-tracker/regression-new.test.mjs')
  );
  ...
  const newBulletIdx = lines.findIndex((l) =>
    /^- \[ \] `node scripts\/tests\/unit\/task-tracker\/regression-new\.test\.mjs`/.test(l)
  );
