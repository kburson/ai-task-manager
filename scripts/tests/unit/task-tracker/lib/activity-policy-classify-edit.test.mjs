...
test('classifyEdit: codeGlobReincludes carve-out for tests under excluded paths', () => {
  const policy = {
    codeGlobs: ['scripts/**'],
    codeGlobExcludes: ['scripts/task-tracker/**', 'scripts/gh/**'],
    codeGlobReincludes: ['scripts/tests/**'],
    docGlobs: [],
    testRunners: [],
    buildCommands: [],
  };
  assert.equal(classifyEdit('scripts/tests/unit/task-tracker/foo.test.mjs', policy), 'WRITE_CODE');
  assert.equal(classifyEdit('scripts/tests/integration/gh/bar.test.mjs', policy), 'WRITE_CODE');
});
