...
import { validateDeclarationCmd } from '../../../../task-tracker/lib/proof-marker.mjs';
...
test('validateDeclarationCmd does NOT reject real commands that merely contain a sentinel word as a substring', () => {
  assert.equal(
    validateDeclarationCmd('node scripts/tests/unit/task-tracker/wip-cleanup.test.mjs'),
    null,
...
test('findAcsWithoutVerifierOrInvalidTag still passes a real backtick-wrapped verifier command', () => {
  const body = [
    '## Acceptance Criteria',
    '- [ ] Some AC <!-- aitm-verified cmd="`node --test scripts/tests/unit/task-tracker/body-invariants.test.mjs`" -->',
    '## Definition of Done',
  ].join('\n');
  assert.deepEqual(
