...
import { computeActiveByPhaseSpans as reexported } from '../../../../task-tracker/active-time.mjs';
import { rollupTotals, parseTimingRows } from '../../../../task-tracker/timing-rollup.mjs';
...
test('AC5 — the read-side calculator has no branch keyed on idle/active-work', () => {
  const src = stripComments(
    readFileSync(
      fileURLToPath(new URL('../../../../task-tracker/lib/timing-rows.mjs', import.meta.url)),
      'utf8'
    )
  );
...
test('AC5 — rollupTotals has no branch keyed on idle/active-work either', () => {
  const src = stripComments(
    readFileSync(
      fileURLToPath(new URL('../../../../task-tracker/timing-rollup.mjs', import.meta.url)),
      'utf8'
    )
  );
