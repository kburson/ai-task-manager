...
While this ai skill project is a public repo is currently under construction and has not been published yet. The rules and templates for interacting with github issue backlogs are under constant revision. We are "dogfooding" the skill as we build it and patch every hole. The design is evolving naturally on every iteration. As soon as the shape of the skill is stabilized we will publish as an npm package.
