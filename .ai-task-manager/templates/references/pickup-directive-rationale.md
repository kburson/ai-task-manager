...
`.ai-task-manager/templates/pickup-directive.md`. The directive itself states each rule
...
dive). Every AC must cite one or more stable root Verification Command IDs via
an `aitm-verified vc-list="vc:N"` HTML comment marker. Every cited ID must
resolve to a command checkbox under the issue's single root
`## Verification Commands` section. Functional DoD items retain their literal
command declarations; ACs do not use that grammar.
Never bulk-check. Never check preemptively. "It looks done" is not
verification.

**Stage ownership (#1356).** Develop may run its allowed declared verifier;
`/task test` owns Test execution. After `/task test` writes a valid receipt,
Test-stage stamps reuse it. Review stamps only reuse a validated exact-SHA
Test receipt and refuse every uncovered command. Missing/stale/red receipt →
demote and `/task test`; never spawn from Review. Named finding only:
`/task review --probe "<command>"`.
...
follow [`.ai-task-manager/templates/session-boot.md`](../session-boot.md) before
