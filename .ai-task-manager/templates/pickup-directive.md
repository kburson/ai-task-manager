...
2. **Verify each DoD/AC.** Use `aitm-verified cmd="…"` in Develop or via `/task test` (`### Verification Commands`); Test stamps reuse its receipt. Review stamps reuse or refuse; never execute. Never bulk-check.
