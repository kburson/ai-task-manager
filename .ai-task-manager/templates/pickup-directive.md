...
2. **Verify every DoD/AC item individually.** In Develop/Test, run its `aitm-verified cmd="…"` command (also listed under `### Verification Commands`), then check the box. In Review, reuse the Test receipt — do not re-run standard lanes (`npm test`, `npm run test:slow`, `npm run test:all`). Never bulk-check.
