...
2. **Verify every DoD/AC item individually.** Run its `aitm-verified cmd="…"` in Develop/Test (`### Verification Commands`). Review reuses the Test receipt — no standard-lane rerun. Never bulk-check.
