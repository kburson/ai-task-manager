...
2. **Verify every DoD/AC item individually.** Develop may run its allowed declared verifier; `/task test` owns the Test execution. After `/task test` writes a valid receipt, Test-stage stamps reuse it. Review stamps only reuse that receipt and refuse every uncovered command; `/task review --probe` is the sole Review execution path. Never bulk-check.
