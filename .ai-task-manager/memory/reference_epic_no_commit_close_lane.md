...
description: 'Epics have no direct commits — close them via `npx aitm kind <N> epic` + deliverable comment + `aitm-deliverable-posted` body marker, never commit-trace.'
metadata:
  node_type: memory
  type: reference
...
2. Post a deliverable summary comment (child issues Done + evidence), then write `<!-- aitm-deliverable-posted url="<comment-url>" -->` into the body via mutateIssueBody.
