---
name: feedback_xl_epics_standalone
description: Epics are titled with the "🧑‍🧒‍🧒 [Epic]" prefix, not "EPIC:"; sub-epics under a root epic are allowed.
metadata:
  node_type: memory
  type: feedback
...
The correct, current title prefix for an epic is `🧑‍🧒‍🧒 [Epic]` (with a
trailing space before the title: the family emoji + `[Epic]`). The historical
`EPIC:` text-prefix convention is **retired** — do not inject it and do not
"fix" an emoji-prefixed title to add it.
