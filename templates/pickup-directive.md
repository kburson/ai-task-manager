...
3. **Append the deep dive to the issue body, then stamp the completion marker.** Full procedure (placement gate, `--body-file` requirement, required subsections, AC `aitm-verified-by` binding, `### Verification Commands`, Dependency Map) is in [`references/deep-dive-procedure.md`](./references/deep-dive-procedure.md). The canonical writer is `stampDeepDive (scripts/task-tracker/lib/deep-dive.mjs)` — one transactional `mutateIssueBody` call that appends the appendix AFTER the Pickup Directive block AND stamps the `<!-- aitm-deep-dive-posted: ... -->` marker (the first of `planDeepDiveGate`'s three signals). Do not hand-write the posted marker. After `stampDeepDive` returns, run `/task check "Deep dive complete"`, which writes the hidden `<!-- aitm-deep-dive-complete: ... -->` marker (the second signal). No visible checkbox is needed.
